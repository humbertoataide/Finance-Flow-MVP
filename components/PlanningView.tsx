...
import { Target, AlertCircle, CheckCircle2, TrendingUp, Zap, Plus, Trash2, Repeat, CalendarCheck, Lightbulb } from 'lucide-react';
...
const PlanningView: React.FC<PlanningViewProps> = ({ 
  ...
  const quickSuggestions = [
    // Receitas
    { label: 'Salário', type: 'income', cat: 'cat-outras-receitas', desc: 'Salário Mensal' },
    { label: 'Stocks/Dividendos', type: 'income', cat: 'cat-investimento', desc: 'Rendimento Stocks' },
    { label: 'Benefícios', type: 'income', cat: 'cat-outras-receitas', desc: 'Vale Alimentação/Refeição' },
    // Despesas
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      ...
      <section className="space-y-4">
        ...
        {!showRecurringForm && recurring.length === 0 && (
          <div className="p-8 bg-white rounded-[2rem] border border-slate-200 border-dashed text-center">
            <div className="max-w-2xl mx-auto space-y-4">
              ...
              <h4 className="font-bold text-slate-800">Crie seus templates fixos</h4>
              <p className="text-sm text-slate-500">Adicione itens que se repetem todo mês para facilitar o controle e automação.</p>
        ...
        {showRecurringForm && (
          <div className="bg-white p-6 rounded-[2rem] border-2 border-blue-100 shadow-xl shadow-blue-50 animate-in slide-in-from-top-4">
            <form onSubmit={handleAddRecurring} className="grid grid-cols-1 md:grid-cols-6 gap-4 items-end">
      ...
      <section className="space-y-4">
        <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
           <Target className="w-4 h-4" /> Metas de Orçamento Mensal
        </h3>
            ...
            return (
              <div key={cat.id} className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm hover:shadow-xl hover:border-blue-100 transition-all duration-300 group">
                <div className="flex items-center justify-between mb-6">
                  ...
                  {isOver && <div className="p-2 bg-rose-50 rounded-xl"><AlertCircle className="w-5 h-5 text-rose-500" /></div>}
                </div>
                ...
                <div className="space-y-5">
                  ...
                  <div className="pt-2 space-y-3">
                    <div className="flex justify-between text-xs">
                      <span className="text-slate-500 font-bold">Gasto Atual</span>
